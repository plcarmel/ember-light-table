...
export default Component.extend({
  ...
  _getPosition(pointName) {
    ...
    let r = this.get('ltBody').ltRows()[i];
    let top = r.top();
  ...
  _computePosition: on('didInsertElement', function() {
    ...
    this._updateA();
    this._updateB();
    this.set('boxStyle', this.__boxStyle());
  }),
  ...
  _updateAnchorAdjustment: observer('inverse', 'directionA', 'range.anchorIsB', function() {
    once(this, this.__updateAnchorAdjustment);
  }),
  ...
  __boxStyle() {
    ...
    let r = this.get('ltBody').ltRows()[0];
    let left = r.left();
    let width = r.width();
